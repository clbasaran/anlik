...
public actor SwiftDataSyncService {
    ...
    public func syncHistoryToLocal(_ photos: [PhotoMetadata]) async {
        ...
        do {
            ...
            lastHistoryHash = incomingHash
            lastHistorySyncTime = now
            AppLogger.service.debug("Synchronized \(photos.count) Strips to SwiftData")
        } catch {
            AppLogger.service.error("Failed to sync history: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    public func syncFriendsToLocal(_ friends: [FriendStatus]) async {
        ...
        do {
            ...
            AppLogger.service.debug("Synchronized \(friends.count) Friends to SwiftData")
        } catch {
            AppLogger.service.error("Failed to sync friends: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    public func syncUserToLocal(_ profile: UserProfile) async {
        ...
        do {
            ...
            AppLogger.service.error("Failed to sync user to local DB: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func _deleteStrip(id stripId: String) {
        ...
        do {
            ...
            AppLogger.service.error("Failed to delete strip \(stripId): \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func _clearAllStrips() {
        ...
        do {
            ...
            AppLogger.service.error("Failed to clear strips: \(error.localizedDescription, privacy: .public)")
        }
    }
}
